...
// RecordTaskExecution stores the latest execution record of a task.
func (s *Store) RecordTaskExecution(inst model.Instance) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.records[inst.TaskID] = inst
}

// ClearTaskRecord removes the execution record of a task so a reused id
// never inherits a previous task's history.
func (s *Store) ClearTaskRecord(taskID string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	delete(s.records, taskID)
}

// TaskRecord returns the latest execution record of a task.
func (s *Store) TaskRecord(taskID string) (model.Instance, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	inst, ok := s.records[taskID]
	return inst, ok
}
